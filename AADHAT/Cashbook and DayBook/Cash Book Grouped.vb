        ...
        Try
            ...
            If dt.Rows.Count > 0 Then
                For i = 0 To dt.Rows.Count - 1
                    ...
                    ssql = "SELECT Entrydate, TransType,AccountName,Remark,sum(Amount) as Dr,'0' as Cr from Ledger where DC ='D' " & IIf(cbAccountName.SelectedValue > 0, "and AccountID=" & Val(cbAccountName.SelectedValue) & "", "") & " and EntryDate = '" & CDate(dt.Rows(i)("Entrydate")).ToString("yyyy-MM-dd") & "'and Transtype  in ('Reciept')   union all" & _
                " SELECT Entrydate,  TransType,AccountName,Remark,'0' as Dr,sum(Amount) as Cr  from Ledger where Dc='C' " & IIf(cbAccountName.SelectedValue > 0, "and AccountID=" & Val(cbAccountName.SelectedValue) & "", "") & " and EntryDate = '" & CDate(dt.Rows(i)("Entrydate")).ToString("yyyy-MM-dd") & "'  and Transtype in ('Reciept') "
                    tmpDt = clsFun.ExecDataTable(ssql)
                    For j = 0 To tmpDt.Rows.Count - 1
                        dg1.Rows.Add()
                        With dg1.Rows(lastval)
                            If tmpDt.Rows(j)("Dr").ToString() <> "0" Then
                                .Cells(1).Value = "Total Reciept"
                    ...
                    Dim k As Integer
                    Dim sql As String = ""
                    sql = "SELECT Entrydate, TransType,AccountName,Remark,sum(Amount) as Dr,'0' as Cr from Ledger where DC ='D' " & IIf(cbAccountName.SelectedValue > 0, "and AccountID=" & Val(cbAccountName.SelectedValue) & "", "") & " and EntryDate = '" & CDate(dt.Rows(i)("Entrydate")).ToString("yyyy-MM-dd") & "' and Transtype not in ('Reciept')   union all" & _
                           " SELECT Entrydate,  TransType,AccountName,Remark,'0' as Dr,sum(Amount) as Cr  from Ledger where Dc='C' " & IIf(cbAccountName.SelectedValue > 0, "and AccountID=" & Val(cbAccountName.SelectedValue) & "", "") & " and EntryDate = '" & CDate(dt.Rows(i)("Entrydate")).ToString("yyyy-MM-dd") & "' and Transtype not in ('Reciept')  "
                    tmpDt = clsFun.ExecDataTable(sql)
                    For k = 0 To tmpDt.Rows.Count - 1
                        dg1.Rows.Add()
                        With dg1.Rows(lastval)
                            If tmpDt.Rows(k)("Dr").ToString() <> "0" And tmpDt.Rows(k)("Dr").ToString() <> "" Then
                                .Cells(1).Value = "Cash Sale"
                                .Cells(2).Value = tmpDt.Rows(k)("TransType").ToString()
                                .Cells(3).Value = tmpDt.Rows(k)("Dr").ToString()
                                drtotal = Val(drtotal) + Val(.Cells(3).Value)
                                If i = 0 And tmpopbaladd = False Then
                                    drtotal = Val(drtotal) + Val(Val(opbal) + Val(tmpamt))
                                    tmpopbaladd = True
                                End If
                            ElseIf tmpDt.Rows(k)("Cr").ToString() <> "0" And tmpDt.Rows(k)("Cr").ToString() <> "" Then
                                .Cells(5).Value = "Total Payment"
                                .Cells(6).Value = tmpDt.Rows(k)("TransType").ToString()
                                .Cells(7).Value = tmpDt.Rows(k)("Cr").ToString()
                                crtotal = crtotal1 + crtotal + Val(.Cells(7).Value)
                                If i = 0 And tmpopbaladd = False Then
                                    crtotal = Val(crtotal) + Val(Val(opbal) - Val(tmpamt))
                                    tmpopbaladd = True
