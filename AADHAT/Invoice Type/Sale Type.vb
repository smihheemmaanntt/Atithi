Public Class Sale_Type
    ...
    Private Sub save()
        ...
        Else
            Dim sql As String = "insert into SaleType(InvoiceTypeName,AccountID,Taxtype,Price,tag)" _
                                & "values (@1, @2, @3, @4,@5)"
            Try
    ...
    Public Sub FillControls(ByVal id As Integer)
        ...
        sSql = "SELECT * FROM SaleType AS IT INNER JOIN Accounts AS Ac ON IT.AccountID = Ac.ID where IT.ID=" & id
        clsFun.con.Open()
    ...
    Private Sub Update()
        ...
        Else
            ...
            ElseIf RadioCompostion.Checked Then
                Radio = "TC"
            End If
            Dim sql As String = "Update SaleType SET InvoiceTypeName='" & txtInvocieTypeName.Text & "', AccountID=" & Val(cbAccount.SelectedValue) & ",Taxtype='" & Radio & "',Price='" & cbPrice.Text & "' WHERE ID=" & Val(txtid.Text) & ""
            cmd = New SQLite.SQLiteCommand(sql, clsFun.GetConnection())
            Try
    ...
    Private Sub Delete()
        Try
            If clsFun.ExecScalarInt("Select tag From SaleType  WHERE ID=" & txtid.Text & "") = 0 Then MsgBox("Pre-Define Master. You Can't Delete it.", vbCritical + vbOKOnly, "Access Denied") : Exit Sub
            If MessageBox.Show("Are you Sure to Delete Sale Type ??", "DELETE", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning) = Windows.Forms.DialogResult.OK Then
                If clsFun.ExecNonQuery("DELETE from SaleType WHERE ID=" & txtid.Text & "") > 0 Then
